...
def test_cloud_sync_detects_onedrive() -> None:
    p = Path(r"C:\Users\Bob\OneDrive\IP-LIVE\.tools\windows\jdk-21\bin\java.exe")
...
def test_cloud_sync_detects_dropbox() -> None:
    p = Path("/Users/me/Dropbox/IP-LIVE/jdk-21/bin/java")
...
def test_cloud_sync_clean_for_local_disk() -> None:
    assert detect_cloud_sync_folder(Path("/Applications/IP-LIVE/jdk-21/bin/java")) == ""
    assert detect_cloud_sync_folder(Path(r"C:\IP-LIVE\jdk-21\bin\java.exe")) == ""
...
def test_probe_timeout_message_mentions_cloud_when_path_in_cloud(
    ...
    placeholder file."""
    onedrive = tmp_path / "OneDrive" / "IP-LIVE" / ".tools" / "windows" / "jdk-21"
...
def test_path_has_non_ascii_detects_thai() -> None:
    ...
    p = Path(r"C:\Users\ปอร์เช่\Downloads\IP-LIVE\jdk-21\bin\java.exe")
    assert lspatch_pipeline._path_has_non_ascii(p) is True


def test_path_has_non_ascii_clean_for_ascii_path() -> None:
    p = Path(r"C:\Users\bob\IP-LIVE\jdk-21\bin\java.exe")
