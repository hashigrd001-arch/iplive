...
class TestToolsRootBase:
    ...
    def test_frozen_mode_uses_project_root(self, tmp_path):
        ...
        install_dir = tmp_path / "IPLive-install"
        install_dir.mkdir()
    ...
    def test_frozen_mode_does_NOT_walk_up(self, tmp_path):
        ...
        install_dir = tmp_path / "anywhere" / "IP LIVE"
    ...
    def test_macos_app_inside_customer_zip_finds_tools(self, tmp_path):
        ...
        IP-LIVE.app → app → bundle) to reach ``.tools/``. The 1.8.2
        bounded-3-level walk stopped at ``IP-LIVE.app/`` and
        ...
        """
        bundle = tmp_path / "IP-LIVE-customer-macos-1.8.4"
        tools_dir = bundle / ".tools"
        tools_dir.mkdir(parents=True)
        macos_dir = bundle / "app" / "IP-LIVE.app" / "Contents" / "MacOS"
    ...
    def test_walk_does_not_escape_to_user_home(self, tmp_path):
        ...
        the dev's workspace, or someone else's IP LIVE install).
        """
...
class TestEnvOverride:
    """``IPLIVE_TOOLS_ROOT`` short-circuits canonical resolution
    ...
    def test_env_var_supersedes_bundled_layout(self, tmp_path):
        ...
        env = {"IPLIVE_TOOLS_ROOT": str(override)}
        with mock.patch.object(sys, "frozen", True, create=True), \
             mock.patch("platform.system", return_value="Windows"), \
            ...
            assert found == ov_adb.resolve(), (
                "IPLIVE_TOOLS_ROOT must beat the bundled layout"
            )
