...
customer pings Line OA we walk them through:
    ...
    1. Open ``%LOCALAPPDATA%\\IP LIVE\\logs\\startup-diagnostic.txt``
    2. Send the file.
...
def test_default_log_dir_is_data_root_logs(tmp_path):
    ...
    ``DATA_ROOT/logs/`` — same place as ``iplive.log``.
    """
...
def test_walks_up_for_dot_tools(tmp_path):
    ...
    ``app/IP-LIVE.exe`` instead of ``run.bat``), the resolver
    must still find it.
    """
    import importlib
    from src import config

    bundle = tmp_path / "IP-LIVE-bundle"
    app_dir = bundle / "app"
    ...
    with mock.patch.object(sys, "frozen", True, create=True), \
        ...
        assert base == bundle, (
            ...
            f"the portable ZIP for users who launched app/IP-LIVE.exe."
        )
