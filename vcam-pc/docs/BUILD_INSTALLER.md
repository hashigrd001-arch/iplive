...
1. สร้าง repo บน GitHub (private OK):
   ```bash
   cd ~/livemobillrerun
   git remote add origin git@github.com:<USERNAME>/ip-live.git
...
3. ภายใน ~10 นาที GitHub Actions จะ build เสร็จ:
   - เข้า https://github.com/<USERNAME>/ip-live/releases
   - จะเห็น release `v1.4.6` พร้อมไฟล์ทั้ง 4:
     - `IP-LIVE-Setup-1.4.6.exe` ← ส่งลูกค้า Windows
     - `IP-LIVE-1.4.6.dmg` ← ส่งลูกค้า Mac
     - `IP-LIVE-customer-windows-1.4.6.zip` ← portable
     - `IP-LIVE-customer-macos-1.4.6.zip` ← portable
...
ใช้เวลา ~3 นาที — output ที่ `vcam-pc\dist\installer\IP-LIVE-Setup-*.exe`
...
```bash
cd vcam-pc
python3 tools/build_pyinstaller.py    # สร้าง IP-LIVE.app
bash tools/build_dmg.sh                # ห่อเป็น .dmg
```

Output ที่ `vcam-pc/dist/installer/IP-LIVE-<version>.dmg`
...
unzip -l IP-LIVE-customer-windows-*.zip | grep -E "(\.private_key|gen_license|init_keys|build_release|license_history)"
