; IP LIVE -- Windows installer (Inno Setup 6.x)
;
; What this builds
; ----------------
; A single self-contained ``IP-LIVE-Setup-<version>.exe`` that
...
;   3. Installs to ``%LOCALAPPDATA%\IP LIVE`` -- per-user, no UAC
...
;   4. Lays down ``IP-LIVE.exe`` (PyInstaller bundle) + ``.tools\``
...
;   6. Registers an uninstaller (Settings ▸ Apps ▸ IP LIVE).
;
...
; Build steps
; -----------
;   1. ``python tools\build_pyinstaller.py``       (produces IP-LIVE.exe)
;   2. ``ISCC tools\installer.iss``                (compiles the installer)
;
; Output: ``vcam-pc\dist\installer\IP-LIVE-Setup-<version>.exe``
;
...
#endif
#define MyAppPublisher "IP LIVE Co., Ltd."
#define MyAppURL       "https://line.me/R/ti/p/@iplive"
#define MyAppExeName   "IP-LIVE.exe"
#define MyOutputName   "IP-LIVE-Setup-" + MyAppVersion
