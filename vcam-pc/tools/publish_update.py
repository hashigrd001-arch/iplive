#!/usr/bin/env python3
"""IP LIVE -- admin helper to package, sign, and publish an
auto-update patch.
...
3. The script writes two files into ``dist/updates/``:
   ...
   * ``iplive-src-1.5.1.zip`` -- the patch (just the ``src/``
...
4. Upload BOTH files to whatever HTTPS host serves
   ...
   Pages the easy path is::
       ...
       cp dist/updates/manifest.json iplive.github.io/updates/manifest.json
       cp dist/updates/iplive-src-1.5.1.zip iplive.github.io/updates/
...
def main(argv: list[str] | None = None) -> int:
    ...
    p.add_argument(
        ...
        default="https://iplive.github.io/updates/iplive-src-{version}.zip",
        help=(
    ...
    patch_path = out_dir / f"iplive-src-{new_version}.zip"
