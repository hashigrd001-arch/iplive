"""PyInstaller entry stub for IP LIVE.
...
when the customer double-clicks ``IP-LIVE.exe`` / ``IP-LIVE.app``:
...
def _show_fatal(text: str) -> None:
    ...
    try:
        ...
        root = tk.Tk()
        root.withdraw()
        messagebox.showerror("IP LIVE — เกิดข้อผิดพลาด", text)
        root.destroy()
    except Exception:
...
def main() -> int:
    ...
    try:
        ...
        _show_fatal(
            ...
            "กรุณาส่งข้อความนี้ให้แอดมินทาง Line @iplive"
        )
        return 1
    ...
    try:
        ...
        _show_fatal(
            ...
            f"{traceback.format_exc()}\n\n"
            "ส่งข้อความนี้ให้แอดมินทาง Line @iplive"
        )
        return 2
