@echo off
REM IP LIVE -- one-command Windows installer build.
REM
...
REM
REM Output:
REM   vcam-pc\dist\installer\IP-LIVE-Setup-<version>.exe
REM
...
echo.
echo  ============================================================
echo   IP LIVE -- Windows Installer Build
echo  ============================================================
echo.
...
REM ---- 2. PyInstaller bundle (IP-LIVE.exe) -------------------
echo.
...
if not exist dist\pyinstaller\IP-LIVE.exe (
    echo  [!] dist\pyinstaller\IP-LIVE.exe was not produced.
    exit /b 1
)
echo  [2/4] OK -- dist\pyinstaller\IP-LIVE.exe
...
echo   DONE.
echo  ============================================================
for %%F in (dist\installer\IP-LIVE-Setup-*.exe) do (
    echo   Output: %%F
