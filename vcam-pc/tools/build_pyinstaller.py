#!/usr/bin/env python3
"""IP LIVE — admin helper: package the app as a single-file
``IP-LIVE.exe`` (Windows) or ``IP-LIVE.app`` (macOS) so the
...
* `IP-LIVE.exe` / `IP-LIVE.app` — fallback, "just double-click".
...
Outputs into ``vcam-pc/dist/pyinstaller/<IP-LIVE.app|exe>``.
"""
...
def _platform_args() -> list[str]:
    ...
    if sys.platform == "darwin":
        ...
        return [
            "--windowed",
            "--osx-bundle-identifier", "com.iplive.studio",
        ]
    return ["--windowed"]
...
def _brand_version() -> str:
    """Read ``BRAND.version`` without importing the heavy UI stack."""
    try:
        sys.path.insert(0, str(PROJECT / "src"))
        from branding import BRAND  # type: ignore

        return str(BRAND.version)
    except Exception:  # noqa: BLE001
        return "0.0.0"


def _write_windows_version_file() -> Path | None:
    """Emit a Windows VS_VERSION_INFO resource for the .exe.

    Why this matters for Defender / SmartScreen
    -------------------------------------------
    A PyInstaller onefile with NO version resource is a textbook
    "unknown, unsigned, metadata-less binary" — exactly the shape
    Defender's heuristic engine flags as ``Trojan:Win32/Wacatac`` for
    the first ~24 h of a new build's life. Stamping CompanyName /
    ProductName / FileVersion / OriginalFilename gives the binary a
    real identity, which measurably reduces the false-positive rate
    (it doesn't replace code-signing, but it's the highest-value step
    we can take without an EV certificate). No-op off Windows.

    Returns the path to the generated file, or ``None`` if we couldn't
    write it (the build then proceeds without the resource).
    """
    ver = _brand_version()
    parts = [int(x) for x in (ver.split(".") + ["0", "0", "0", "0"])[:4]
             if x.isdigit()]
    while len(parts) < 4:
        parts.append(0)
    vtuple = tuple(parts[:4])
    content = f"""# Auto-generated by build_pyinstaller.py — do not edit.
VSVersionInfo(
  ffi=FixedFileInfo(
    filevers={vtuple},
    prodvers={vtuple},
    ...
    StringFileInfo(
      [
        StringTable(
          '040904B0',
          [
            StringStruct('CompanyName', 'IP LIVE Co., Ltd.'),
            StringStruct('FileDescription', 'IP LIVE — Live Streaming Studio'),
            StringStruct('FileVersion', '{ver}'),
            StringStruct('InternalName', 'IP-LIVE'),
            StringStruct('LegalCopyright', '\\xa9 IP LIVE Co., Ltd.'),
            StringStruct('OriginalFilename', 'IP-LIVE.exe'),
            StringStruct('ProductName', 'IP LIVE'),
...
def main(argv: list[str] | None = None) -> int:
    ...
    icon_arg: list[str] = []
    version_arg: list[str] = []
    if sys.platform == "win32":
        ico = PROJECT / "assets" / "logo.ico"
        if ico.is_file():
            icon_arg = ["--icon", str(ico)]
        # Stamp a real VS_VERSION_INFO resource — cuts Defender /
        # SmartScreen false positives on the freshly-built exe.
        vf = _write_windows_version_file()
        if vf is not None:
            version_arg = ["--version-file", str(vf)]
