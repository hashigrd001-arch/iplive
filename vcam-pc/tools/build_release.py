...
def _add_prebuilt_app(
    ...
    * The ``Contents/MacOS/<binary>`` entry must keep its executable
    ...
    n = 0
    if os_name == "macos":
        app_dir = prebuilt_dir / "IP-LIVE.app"
        ...
        exe = prebuilt_dir / "IP-LIVE.exe"
        if exe.is_file():
            zf.write(exe, f"{prefix}/app/IP-LIVE.exe")
            n += 1
    return n
...
def _launcher_body(os_name: str) -> str:
    ...
    if os_name == "windows":
        ...
        return textwrap.dedent("""\
            ...
            chcp 65001 > nul
            title IP LIVE
            REM IP LIVE -- Windows launcher (ASCII-only by design)
            cd /d "%~dp0vcam-pc"
            ...
            if "%PYBIN%"=="" (
                ...
                echo    Contact admin via Line OA: @iplive
                echo.
                pause
                exit /b 1
            )
    ...
    if os_name == "macos":
        ...
        return textwrap.dedent("""\
            #!/usr/bin/env bash
            # IP LIVE — macOS launcher (v1.4)
            set -u

            cd "$(dirname "$0")/vcam-pc"

            err_dialog () {
                osascript -e "display dialog \\"$1\\" buttons {\\"OK\\"} default button 1 with icon stop with title \\"IP LIVE\\""
            }
            ...
            echo
            echo "  IP LIVE — กำลังติดตั้งส่วนประกอบครั้งแรก (ใช้ Internet)"
            echo "  อย่าปิดหน้าต่างนี้ ใช้เวลาประมาณ 30 วินาที..."
            echo

            LOG="$TMPDIR/IP-LIVE-install.log"
...
def _windows_install_thai() -> str:
    ...
    return (
        ...
        "\n"
        "ถ้ายังเจอปัญหา ส่งข้อความใน Line OA: @iplive ได้เลยครับ\n"
...
def build_one(target: str, os_name: str, dist: Path) -> Path:
    ...
    with zipfile.ZipFile(
        ...
        #   • Double-click IP-LIVE.app/.exe — works without Python
