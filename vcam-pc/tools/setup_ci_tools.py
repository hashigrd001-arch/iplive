#!/usr/bin/env python3
"""IP LIVE — CI helper: download every tool the customer install
...
# IP LIVE installer". Root cause: Windows ships no Android USB
...
def _resolve_lspatch_url() -> str:
    ...
    Resolution strategy (each step short-circuits the rest):
      ...
      1. ``$IPLIVE_LSPATCH_URL`` env var — operator override.
    ...
    """
    override = os.environ.get("IPLIVE_LSPATCH_URL", "").strip()
    if override:
        print(f"  -> using IPLIVE_LSPATCH_URL override: {override}")
        return override

    print("  -> querying GitHub for latest LSPatch release")
    headers = {
        "Accept": "application/vnd.github+json",
        "User-Agent": "ip-live-ci-setup/1.0",
    }
