...
Color palette: electric blue (#2E9BFF) with a violet accent
(#8B5CF6) on a near-black background (#0F0F14) — matches the IP LIVE
logo (chrome wordmark with a blue→violet play triangle on deep
black) and reads as "premium, broadcast, energetic", which is what
TikTok-Live sellers want.

A lime accent (#A6FF4D) stays reserved for the go-live / record
state so the eye instantly knows when the system is "hot" — bright
lime-green on blue-and-black is the highest-contrast combination
...
class Brand:
    ...
    line_oa: str = "@iplive"
    contact_url: str = "https://line.me/R/ti/p/@iplive"
...
class Theme:
    ...
    # Brand blue — sampled from the IP LIVE logo and tuned for screen
    # contrast against #0F0F14 (passes WCAG AA for body text).
    primary: str = "#2E9BFF"
    primary_hover: str = "#52B0FF"
    primary_dim: str = "#1763B5"

    # Violet accent from the logo's play triangle gradient.
    accent: str = "#8B5CF6"
