...
DEFAULT_MANIFEST_URL = os.environ.get(
    ...
    "https://iplive.github.io/updates/manifest.json",
)
...
_STAGING_DIR_NAME = "iplive_update_staging"
...
def _http_get(url: str, *, timeout: float = 8.0,
    ...
    req = urllib.request.Request(
        url,
        headers={
            "User-Agent": f"IP-LIVE/{BRAND.version}",
...
def download_patch(
    ...
    *,
    ...
    out = staging / f"iplive-src-{manifest.version}.zip"
...
def _patch_filename(manifest: UpdateManifest) -> str:
    ...
    return f"iplive-src-{manifest.version}-{sha_prefix}.zip"
