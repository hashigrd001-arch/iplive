...
Customers buy IP LIVE to *not* have to think about technical
...
* Downloaded from the official Genymobile/scrcpy GitHub release
  ...
  GitHub).
* Stashed under ``~/.iplive/tools/scrcpy-<version>/`` so an
  uninstall is just ``rm -rf`` of the IP LIVE data dir, no
...
def user_tools_root() -> Path:
    """Return ``~/.iplive/tools`` — owner of every auto-installed
    binary. Mirrors the path family already used by
    ``license_key.ACTIVATION_PATH = ~/.iplive/activation.json`` so
    the customer's data lives under one well-known root they (or
    we) can backup / wipe / inspect.
    """
    return Path.home() / ".iplive" / "tools"
...
def find_user_installed() -> Optional[Path]:
    """Return Path to scrcpy binary inside ``~/.iplive/tools/``,
...
def _http_download(
    ...
    try:
        req = urllib.request.Request(
            url,
            headers={"user-agent": "IPLive-installer/1.0"},
        )
...
def install(
    ...
    if asset is None:
        raise InstallerError(
            ...
            "อัพเดท IP LIVE เป็นเวอร์ชันล่าสุดแล้วลองใหม่"
        )
    ...
    if binary is None:
        raise InstallerError(
            ...
            "ลบโฟลเดอร์ ~/.iplive/tools แล้วลองใหม่"
        )
    if progress:
        try:
            progress("done", 1, 1)
