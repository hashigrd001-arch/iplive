"""Entry point for IP LIVE / vcam-pc.
...
def _setup_logging(verbose: bool) -> None:
    ...
    ``logs/iplive.log`` (or the diagnostic ZIP we generate from
...
def _parse_args(argv: list[str]) -> argparse.Namespace:
    p = argparse.ArgumentParser(prog="iplive")
    p.add_argument(
        "--studio",
        action="store_true",
        help="launch the IP LIVE customer UI (default)",
    )
    p.add_argument(
        "--legacy",
        action="store_true",
...
def _check_writable_install_location() -> bool:
    ...
    IP-LIVE.app straight from the mounted .dmg) and refuse to
    boot.
    ...
    IP-LIVE.app while it's still inside the .dmg volume (which
    ...
    probe = PROJECT_ROOT / ".iplive-writable-probe"
    try:
        probe.write_bytes(b"")
    ...
    if looks_like_volume:
        title = "IP LIVE — กรุณาติดตั้งก่อนเปิดใช้งาน"
        message = (
            "ตอนนี้คุณกำลังเปิด IP LIVE จากในแผ่นภาพดิสก์ (.dmg)\n"
            "ซึ่งเป็นพื้นที่อ่านอย่างเดียว ทำให้โปรแกรมเขียน\n"
            "ค่าตั้งค่า / log / cache ไม่ได้\n\n"
            "วิธีติดตั้งให้ถูกต้อง:\n"
            "  1. ลาก  IP-LIVE  ใส่ทางลัด  Applications  ในหน้าต่างเดียวกัน\n"
            "  2. ปิดหน้าต่างนี้ (Eject) แผ่นภาพดิสก์\n"
            "  3. เปิด  IP-LIVE  จาก  /Applications/  (Launchpad / Finder)\n\n"
            "โปรแกรมจะปิดตัวเองตอนนี้ — เปิดอีกครั้งหลังลากเสร็จได้เลยครับ"
        )
    else:
        title = "IP LIVE — กรุณาติดตั้งก่อนเปิดใช้งาน"
        message = (
            "IP LIVE ติดตั้งอยู่ในตำแหน่งที่เขียนไฟล์ไม่ได้:\n  "
            f"{PROJECT_ROOT}"
