...
class HookModePipeline:
    ...
    def _run_ffmpeg_with_progress(
        ...
        compute a 0..1 percentage, and guard against a *stalled*
        encoder with a background watchdog -- killing the child only
        when it stops making progress, never merely because the total
        encode ran long.

        ``timeout_s`` is retained for signature/back-compat but is no
        longer a hard wall-clock cap: capping on total time is what
        made long clips (>~10 min) fail when the duration probe came
        back 0 and the budget collapsed to its 600 s floor. See the
        stall-watchdog comment in the body.

        We never raise from this method -- callers expect a
        ``HookEncodeResult`` regardless of crash mode (stall, OS
        ...
        #
        # Stall watchdog (v1.8.26) — why we no longer cap total time
        # ----------------------------------------------------------
        # The old code killed the encode once ``time.monotonic()``
        # passed ``t0 + timeout_s``. ``timeout_s`` came from
        # ``_encode_timeout(duration_s, …)``, which floors at 600 s
        # (10 min) and, crucially, COLLAPSES to that floor whenever
        # the duration probe returns 0 (no ffprobe + per-file probe
        # failure — a real, handled case, see the heartbeat branch
        # below). The net customer bug: any source clip whose encode
        # took longer than ~10 min died with "ffmpeg เกินเวลา"
        # ("คลิปยาวเกิน 10 นาที encode ไม่ผ่าน").
        #
        # The correct invariant is "kill only if ffmpeg stops making
        # progress", not "kill after N minutes total". So we track the
        # last time the encoded position advanced and let a background
        # watchdog kill the child only after ``stall_timeout_s`` of NO
        # forward progress. A long clip that keeps encoding is never
        # killed, regardless of length. A genuinely hung ffmpeg (no
        # output at all — which would otherwise block this reader on
        # ``for line in proc.stdout``) is still caught, because the
        # watchdog runs on its own thread.
        stall_timeout_s = max(
            1, int(getattr(self.cfg, "encode_stall_timeout_s", 300) or 300)
        )
        progress_ts = {"t": time.monotonic()}
        watchdog_killed = {"flag": False}
        done = threading.Event()

        def _watchdog() -> None:
            poll = max(0.05, min(2.0, stall_timeout_s / 4.0))
            while not done.wait(poll):
                # Cancel is handled by the reader loop; don't double-kill.
                if cancel_event is not None and cancel_event.is_set():
                    return
                if time.monotonic() - progress_ts["t"] > stall_timeout_s:
                    watchdog_killed["flag"] = True
                    try:
                        proc.kill()
        ...
        threading.Thread(
            target=_watchdog, daemon=True, name="ffmpeg-stall-watchdog",
        ).start()
        ...
        # Watchdog fired: ffmpeg stalled (no forward progress). Surface
        # a clear "stalled" cause rather than the old length-based
        # "ตัดคลิปสั้นลง" message — length is no longer the problem.
        if watchdog_killed["flag"]:
            try:
                proc.wait(timeout=5)
            except subprocess.TimeoutExpired:
                pass
            mins = stall_timeout_s // 60
            return HookEncodeResult(
                False, output_path, 0.0, 0,
                f"ffmpeg ค้าง ไม่มีความคืบหน้านานเกิน {mins} นาที จึงหยุดให้\n"
                "• ปิดโปรแกรมที่กินซีพียู/ดิสก์หนัก ๆ แล้วลองใหม่\n"
                "• ตรวจว่าพื้นที่ดิสก์ไม่เต็ม และไฟล์คลิปไม่เสียหาย\n"
                "• ถ้ายังไม่หาย ส่ง log ให้แอดมิน Line @iplive",
            )

        # Wait for the child to actually exit so we can read its
        # return code. The stall watchdog (not a total-time cap) is
        # what guards against a hung encoder, so a generous fixed
        # grace here is safe even for multi-hour encodes.
        try:
            ...
            return HookEncodeResult(
                False, output_path, 0.0, 0,
                "ffmpeg ไม่ยอมจบหลัง encode เสร็จ — ลองใหม่อีกครั้งครับ",
            )
