...
def _tools_root_base() -> Path:
    ...
    * **Inno Setup install** (``installer.iss`` lays ``.tools/`` at
      ``{app}\\.tools\\``, sibling of IP-LIVE.exe). Frozen mode,
    ...
    * **Portable ZIP with PyInstaller bundle in app/** (some
      customers double-click ``app/IP-LIVE.exe`` from inside the
    ...
    * **macOS portable ZIP, .app double-clicked from Finder** —
      ...
      ``<bundle>/app/IP-LIVE.app/`` *plus* the ``run.command``
    ...
    nested ``app/IP-LIVE.app/Contents/MacOS/`` case but shallow
    ...
    # ``Contents/MacOS/`` → ``Contents/`` → ``IP-LIVE.app/`` →
...
def find_vcam_apk() -> Path | None:
    ...
    * PyInstaller frozen — ``apk/`` ships next to IP-LIVE.exe
