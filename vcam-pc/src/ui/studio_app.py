"""IP LIVE — main application shell.
...
class _DevicePoller(threading.Thread):
    ...
    def __init__(
        ...
        super().__init__(daemon=True, name="iplive-device-poller")
        self._adb = adb
...
class _WifiReconnector(threading.Thread):
    ...
    def __init__(self, adb: AdbController, get_targets, stop_event) -> None:
        super().__init__(daemon=True, name="iplive-wifi-reconnect")
        self._adb = adb
    ...
    def _check_adb_or_warn(self) -> None:
        ...
        * Linux   → ``chmod +x`` + libc / executable-flag mismatch
        ...
        if is_windows:
            ...
            help_steps = (
                "1. รีสตาร์ทเครื่อง แล้วเปิด IP LIVE อีกครั้ง\n"
            ...
            help_steps = (
                ...
                f'{adb_name}'
                '"\n   (path เห็นข้างบน) แล้วเปิด IP LIVE อีกครั้ง\n'
