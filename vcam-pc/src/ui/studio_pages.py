...
class DashboardPage(ctk.CTkFrame):
    ...
    def _build_main(self) -> None:
        ...
        #    IP LIVE build (pre-WiFi-auto) need this.
        #
    ...
    def _on_hook_status_done(self, status) -> None:
        ...
        # before IP LIVE started recording the install signature.
    ...
    def _offer_auto_install_then_mirror(
        ...
        ``~/.iplive/tools/`` so an uninstall is just deleting
        that directory.
        """
    ...
    def _on_open_dashboard(self) -> None:
        ...
        try:
            ...
            messagebox.showerror(
                ...
                "จากนั้นเปิดโปรแกรม IP LIVE ใหม่",
            )
        except Exception as exc:
    ...
    def _open_play_store_search(self, serial: str, query: str) -> None:
        ...
        try:
            ...
            messagebox.showinfo(
                ...
                "3. กลับมาที่ IP LIVE แล้วกด Patch ใหม่อีกครั้ง",
            )
        except Exception as ex:
...
class WizardPage(ctk.CTkFrame):
    ...
    def _render_step_1(self) -> None:
        ...
        #   * Patch was applied on an older IP LIVE version
        ...
        # daemon on port 5037 before IP LIVE launched).
        actions = ctk.CTkFrame(wrap, fg_color="transparent")
        actions.grid(row=3, column=0, sticky="w", padx=24, pady=(0, 8))
        _ghost_button(
            actions,
    ...
    def _show_driver_help(self) -> None:
        ...
        who developed/tested IP LIVE on Mac never hit the driver
        ...
        def _open_driver_folder() -> None:
            ...
            if driver_dir is None:
                messagebox.showwarning(
                    ...
                    "กรุณาส่งข้อความให้แอดมิน Line @iplive",
                )
                return
            try:
                if _sys.platform == "win32":
        ...
        if universal_msi is None and driver_dir is None:
            _muted(
                ...
                " ส่งข้อความให้แอดมิน Line @iplive ได้เลยครับ)",
...
class SettingsPage(ctk.CTkFrame):
    ...
    def _build_support_card(self, parent: ctk.CTkFrame) -> None:
        ...
        * **ส่ง Log ให้แอดมิน** -- bundles redacted logs + system
          ...
          ever needing to find ``%APPDATA%\\IPLive\\logs``.
    ...
    def _on_create_backup(self) -> None:
        ...
        out = filedialog.asksaveasfilename(
            ...
            initialfile=suggested,
            initialdir=str(default_dir) if default_dir.exists() else None,
            filetypes=[("IP LIVE backup", "*.zip"), ("All files", "*.*")],
        )
        if not out:
            return
        try:
    ...
    def _on_restore_backup(self) -> None:
        ...
        if manifest is None:
            messagebox.showerror(
                "ไฟล์ไม่ถูกต้อง",
                "ไฟล์นี้ไม่ใช่ Backup ของ IP LIVE (ไม่พบ manifest)\n"
