"""IP LIVE -- logging configuration and diagnostic export.
...
1. Configure a **rotating file handler** that writes structured
   logs to ``<DATA_ROOT>/logs/iplive.log`` for every run (on frozen
   macOS ``DATA_ROOT`` is ``~/Library/Application Support/IP LIVE``
...
LOG_FILE = LOG_DIR / "iplive.log"

# 5 MB per file × 3 backups = ~20 MB ceiling per install. The
# customer's "C:\Program Files\IP LIVE" almost always has more
...
def configure_logging(verbose: bool = False) -> Path:
    ...
    * Sets the root logger to DEBUG so neither handler upstreams
    ...
    for h in list(root.handlers):
        if getattr(h, "_iplive_managed", False):
    ...
    file_h._iplive_managed = True  # type: ignore[attr-defined]
    ...
    console_h._iplive_managed = True  # type: ignore[attr-defined]
    ...
    with zipfile.ZipFile(out_path, "w", zipfile.ZIP_DEFLATED, compresslevel=9) as zf:
        ...
        zf.writestr(
            ...
            (
                ...
                f"  logs/iplive.log*  -- rolling app log\n"
...
def suggest_diagnostic_filename() -> str:
    ...
    return f"iplive-diag-v{BRAND.version}-{ts}.zip"
