"""IP LIVE -- backup and restore the customer's local state.
...
* ``customer_devices.json`` and the legacy ``~/.iplive/devices.json``
...
# Path to the legacy ~/.iplive/devices.json. Saved separately
# because it lives outside PROJECT_ROOT.
_HOME_DEVICES = Path.home() / ".iplive" / "devices.json"
...
def restore_backup(zip_path: Path) -> list[str]:
    ...
    if manifest is None:
        raise ValueError(
            "ไฟล์ไม่ใช่ Backup ของ IP LIVE (manifest หายหรือเสีย)"
        )
    ...
    restored: list[str] = []
    with tempfile.TemporaryDirectory(prefix="iplive-restore-") as td:
        staging = Path(td)
        with zipfile.ZipFile(zip_path) as zf:
...
def _resolve_destination(member: str) -> Optional[Path]:
    ...
    * ``home/devices.json`` → ``~/.iplive/devices.json``
...
def suggest_backup_filename() -> str:
    """``iplive-backup-v1.5.0-20260508-2330.zip``."""
    ts = datetime.now().strftime("%Y%m%d-%H%M")
    return f"iplive-backup-v{BRAND.version}-{ts}.zip"
