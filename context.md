...
Deep architecture reference for **IP LIVE** (`livemobillrerun` monorepo).
...
<workspace>/
  ...
  vcam-pc/logs/                ← iplive.log + startup-diagnostic.txt
...
.github/workflows/release.yml  (Python 3.13)
   ├─ Windows runner: build_pyinstaller.py → IP-LIVE.exe
   │                  ISCC installer.iss   → IP-LIVE-Setup-<v>.exe
   │                  build_release.py      → ...customer-windows-<v>.zip
   └─ macOS runner:   build_pyinstaller.py → IP-LIVE.app
                      build_dmg.sh          → IP-LIVE-<v>.dmg
