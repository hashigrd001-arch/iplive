# Deploying IP LIVE — License & Admin Server
...
* A domain name (or subdomain) like `admin.ip-live.com`.
...
```
admin.ip-live.com    A    <VPS-IPv4>
```

Wait until `dig admin.ip-live.com` answers with the right IP
...
```bash
mkdir -p /opt && cd /opt
git clone <your-git-remote> iplive
cd iplive/vcam-server
```
...
```bash
# On your laptop:
scp -r vcam-server root@<VPS-IP>:/opt/iplive/
```

## Step 4 — configure

```bash
cd /opt/iplive/vcam-server
cp .env.example .env
nano .env
# Set DOMAIN=admin.ip-live.com (your real subdomain)
...
HTTPS request. Open `https://admin.ip-live.com/admin` in your
browser and log in.
...
```python
license_server_url = "https://admin.ip-live.com"
```
...
tar czf /backups/npc-$(date +%F).tar.gz /opt/iplive/vcam-server/data
```

The `data/` dir contains:
* `iplive.sqlite3`  — all customers, licenses, payments.
