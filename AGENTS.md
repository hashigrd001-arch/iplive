...
**IP LIVE** — a Thai-market desktop app (`vcam-pc/`) for TikTok Live
...
**macOS**. Support is via **Line OA: @iplive**. See
...
- Read the top docstring of the module you're editing first —
  ...
  block at the top.
- Customer logs land at `vcam-pc/logs/iplive.log` +
...
- **v1.8.26** — Rebrand NP Create → **IP LIVE** (name, paths
  ...
  the "คลิปยาวเกิน 10 นาที encode ไม่ผ่าน" bug. The hook encode used a
  total wall-clock cap (`hook_mode._run_ffmpeg_with_progress`,
  `deadline = t0 + timeout_s`); when the duration probe returned 0 the
  budget collapsed to its 600 s (10 min) floor, so any clip whose
  encode ran past ~10 min was killed with "ffmpeg เกินเวลา". Replaced
  with a **stall watchdog**: a clip that keeps making progress is never
  killed regardless of length; only a genuinely stalled ffmpeg (no
  forward `out_time` for `StreamConfig.encode_stall_timeout_s`, default
  300 s) is. Tests: `tests/test_hook_progress.py`
  (`test_long_encode_not_killed_while_progressing`,
  `test_stall_watchdog_kills_hung_encode`).
